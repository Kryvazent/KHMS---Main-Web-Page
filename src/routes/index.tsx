  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-20 md:px-8 md:py-24">
      <div className="grid gap-12 md:grid-cols-2 md:items-center">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--color-crimson)]">About EMergeSL</p>
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <Link to="/about" className="inline-flex items-center gap-2 rounded-full bg-[var(--color-navy)] px-5 py-2.5 text-sm font-bold text-white hover:bg-[var(--color-navy)]/90">
              Our Story <ArrowRight className="h-4 w-4" />
            </Link>
            <Link to="/teaching" className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-5 py-2.5 text-sm font-bold text-foreground hover:bg-secondary">
              Teaching Hub
            </Link>
          </div>
        </div>
    ...
    <section className="bg-secondary/40 py-20 md:py-24">
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--color-crimson)]">Explore everything</p>
            <h2 className="mt-2 text-3xl font-black tracking-tight md:text-4xl">All Content Areas</h2>
            <p className="mt-3 max-w-2xl text-muted-foreground">Every section of EMerge SL — teaching, workshops, protocols, disaster response, research, electives, AI tools and more.</p>
          </div>
        </div>
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {sections.map((s) => {
            ...
            return (
              <Link key={s.id} to={s.route} className="group flex flex-col rounded-2xl border border-border bg-card p-6 transition-all hover:-translate-y-1 hover:shadow-xl">
                <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl text-white" style={{ backgroundColor: accent }}>
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-24 md:px-8 md:py-32">
        <div className="max-w-3xl">
          ...
          <h1 className="text-5xl font-black leading-[1.05] tracking-tight md:text-7xl">
            Emergency Care for the <span className="text-[var(--color-crimson-soft)]">Real World</span>.
          </h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link to="/resources" className="inline-flex items-center gap-2 rounded-full bg-[var(--color-crimson)] px-6 py-3 text-sm font-bold text-white shadow-lg shadow-[var(--color-crimson)]/30 transition-transform hover:-translate-y-0.5">
...
function Validation() {
  const items = [
    { icon: Wrench, title: "Frugal Environments", body: "Workarounds and DIY solutions for equipment shortages." },
    { icon: Users, title: "Task-Shifting", body: "Clear guidance designed for clinical officers, nurses, and medical assistants." },
    { icon: Layers, title: "Resource Stratification", body: "Protocols that scale from a basic rural clinic to a regional referral center." },
  ];
  return (
    <section className="mx-auto max-w-7xl px-4 py-20 md:px-8 md:py-24">
      <div className="max-w-3xl">
  ...
  return (
    <section className="bg-secondary/40 py-20 md:py-24">
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {cols.map((c) => (
            <div key={c.title} className="flex flex-col rounded-2xl border border-border bg-card p-7 shadow-sm">
              ...
              <Link to={c.to} className="mt-6 inline-flex items-center gap-1.5 text-sm font-bold text-[var(--color-crimson)]">
...
function QuickAccess() {
  const cards = [
    ...
    { tag: "Respiratory", title: "How to Build a CPAP System from Basic Ward Supplies", to: "/resources" },
    { tag: "POCUS", title: "POCUS Guide: The eFAST Exam in Under 3 Minutes", to: "/resources" },
    { tag: "Triage", title: "South African Triage Scale (SATS) Implementation Toolkit", to: "/resources" },
  ];
  return (
    <section className="mx-auto max-w-7xl px-4 py-20 md:px-8 md:py-24">
      <h2 className="text-3xl font-black tracking-tight md:text-4xl">High-Yield Quick Reference</h2>
      <p className="mt-3 max-w-2xl text-muted-foreground">The protocols our users open most — straight to the algorithm.</p>
      <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {cards.map((c, i) => (
          <Link key={i} to={c.to} className="group flex flex-col justify-between rounded-2xl border border-border bg-card p-6 transition-all hover:-translate-y-1 hover:border-[var(--color-navy)]/40 hover:shadow-xl">
            <div>
...
function Community() {
  return (
    <section className="relative overflow-hidden bg-[var(--color-navy-deep)] text-white" id="offline-app">
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-20 md:px-8 md:py-24">
        <div className="grid gap-12 md:grid-cols-2 md:items-center">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/research" className="inline-flex items-center gap-2 rounded-full bg-[var(--color-crimson)] px-6 py-3 text-sm font-bold text-white shadow-lg">
                Submit a Case Study <ArrowRight className="h-4 w-4" />
              </Link>
              <Link to="/contact" className="inline-flex items-center gap-2 rounded-full border border-white/30 bg-white/5 px-6 py-3 text-sm font-bold text-white hover:bg-white/10">
